...
function initializeEntryPage() {
    ...
    entryButton.addEventListener('click', function() {
        ...
        if (backgroundMusic && !isMuted) {
            ...
            backgroundMusic.volume = 0.3; // Set volume to 30% for background music
...
function initializeMainSite() {
    ...
    updateSpotifyUI({
        ...
        timestamps: {
            start: now, // Start from now (0 seconds)
            end: now + songDuration // End in 3:39
        }
    });
...
async function updateDiscordStatus() {
    try {
        console.log('Fetching Discord status for user:', CONFIG.DISCORD_USER_ID);
        ...
        if (data.success && data.data) {
        ...
        } else if (data.error && data.error.code === 'user_not_monitored') {
            console.log('User not monitored by Lanyard, showing default online status');
            // Show default online status when user is not monitored
            updateDiscordUI({
            ...
            // Also update Spotify to show Teenage Fever by Drake
            const now = Date.now();
            const songDuration = (3 * 60 + 39) * 1000; // 3 minutes 39 seconds in milliseconds
            ...
            console.log('Discord API returned no data or error:', data);
...
function updateDiscordUI(userData) {
    ...
    console.log('Updating Discord UI with data:', userData);
...
async function updateSpotifyStatus() {
    try {
        ...
        if (data.success && data.data && data.data.spotify) {
            ...
            // Keep showing the fake Spotify data when user is not monitored
            console.log('User not monitored, keeping fake Spotify data');
            // Don't call updateSpotifyUI(null) here - keep the fake data
        } else {
